...
def get_all_one_tail_global_adjusted_p_vals_files(all_top_dirs_containing_raw_p_vals_csvs=[overall_top_public_data_dir,overall_top_syn_data_dir]):
    ...
    for top_dataset_dir in all_top_dirs_containing_raw_p_vals_csvs:
        for subdir in report_subdirs_recursively(dir_=top_dataset_dir,absolute_names=True):
            #===========================
            if 'All_EPs_AD_P_vals' in subdir:
                print(f'Skip this directory (from a previous run of merge_default_method_res_for_all_eps.py) to avoid double counting p-values: {subdir}')
                continue
            #============================
...
def update_counts(row,counts_dict,sig_level=0.05):
    if pd.isna(row['GLOBAL Adjusted P-value']):
        ...
        counts_dict['total_no_scenarios_for_which_two_tail_p_vals_were_computed'] += 1

        if round(row['GLOBAL Adjusted P-value'],2) <= sig_level:
            counts_dict['total_no_with_adjusted_two_tail_stat_sig'] += 1
...
def count_p_vals_of_interest(all_one_tail_global_adjusted_p_vals_files):
    ...
    total_no_scenarios_for_which_two_tail_p_vals_were_computed = counts_dict['total_no_scenarios_for_which_two_tail_p_vals_were_computed']
    ...
    total_no_with_adjusted_two_tail_stat_sig = counts_dict['total_no_with_adjusted_two_tail_stat_sig']
    ...
    return total_no_scenarios_for_which_two_tail_p_vals_were_computed,total_no_with_two_tail_adjusted_stat_sig_when_average_shift_metric_had_the_wrong_sign,total_no_with_adjusted_two_tail_stat_sig
...
def main():
    ...
    total_no_scenarios_for_which_two_tail_p_vals_were_computed,total_no_with_two_tail_adjusted_stat_sig_when_average_shift_metric_had_the_wrong_sign,total_no_with_adjusted_two_tail_stat_sig = count_p_vals_of_interest(all_one_tail_global_adjusted_p_vals_files)

    print(f'total_no_scenarios_for_which_two_tail_p_vals_were_computed={total_no_scenarios_for_which_two_tail_p_vals_were_computed}')
    ...
    print(f'total_no_with_adjusted_two_tail_stat_sig={total_no_with_adjusted_two_tail_stat_sig}')
    print(f'Proportion={round(100*(total_no_with_two_tail_adjusted_stat_sig_when_average_shift_metric_had_the_wrong_sign/total_no_with_adjusted_two_tail_stat_sig),2)}')
