...
def report_stats_for_model_ready_dataset(update_version,selected_uncertainty_methods,selected_ad_methods,rand_seed,min_no_cmpds_per_class=min_no_cmpds_per_class,id_col=id_col,smiles_col=smiles_col,class_label_col=class_label_col,class_1=class_1,original_class_1=original_class_1,original_class_0=original_class_0):
    
    print(f'Running report_stats_for_model_ready_dataset(...) for update_version={update_version},random seed={rand_seed}')
    ...
    print(f'no. class-0 in training set={len([v for v in train_inc_calib_y if 0==v])}')
    print(f'no. class-1 in training set={len([v for v in train_inc_calib_y if 1==v])}')
    print(f'class-balance fraction in training set={len([v for v in train_inc_calib_y if 0==v])/len([v for v in train_inc_calib_y if 1==v]):.2f}')

    print(f'no. in test set={len(test_y)}')
    print(f'no. class-0 in test set={len([v for v in test_y if 0==v])}')
    print(f'no. class-1 in test set={len([v for v in test_y if 1==v])}')
    print(f'class-balance fraction in test set={len([v for v in test_y if 0==v])/len([v for v in test_y if 1==v]):.2f}')
...
def main():
    ...
    for update_version in updates_in_order:
        ...
        for global_random_seed in all_global_random_seed_opts:
            ...
            report_stats_for_model_ready_dataset(update_version,selected_uncertainty_methods,selected_ad_methods,rand_seed=global_random_seed)
