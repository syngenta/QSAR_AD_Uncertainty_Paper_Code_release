...
def modellingEvalAnalysisForOneModelVersion(update_version,selected_uncertainty_methods,selected_ad_methods,ADMethod2Param,non_conformity_scaling,ml_alg,number_of_scp_splits,number_of_acp_splits,icp_calib_fraction,nrTrees,global_random_seed,id_col,smiles_col,ep_col,dataset_file,min_no_cmpds,sig_level_of_interest,all_sig_levels_considered_to_compute_ECE,native_uncertainty_alg_variant):
    print(f'Running modellingEvalAnalysisForOneModelVersion(...) for update_version={update_version},random seed={global_random_seed}')
    ...
    for uncertainty_method in selected_uncertainty_methods:
        ...
        for AD_method_name in selected_ad_methods:
            ...
            test_id_status_dict = findInsideOutsideADTestIds(X_train=X_train_and_ids_df, X_test=X_test_and_ids_df, fps_train=fps_train, fps_test=fps_test, threshold=ADMethod2Param[AD_method_name], id_col=id_col,rand_seed=global_random_seed, endpoint_col=ep_col, AD_method_name=AD_method_name,test_ids=test_ids, y_train=train_inc_calib_y, y_test=None, regression=True,class_1_label=None,class_0_label=None)
            ...
            for AD_subset in ['All','Inside','Outside']:
                ...
                if not 0 == len(subset_test_ids):
                    ...
                    updateDictOfRawInsideVsOutsideADResults(dict_of_raw_results,AD_method_name,uncertainty_method,AD_subset,subset_test_ids,subset_test_y,subset_test_predictions,subset_sig_level_2_prediction_intervals)
                    ...
                    RegEval.plotPredPlusEstimatedErrorBarsVsExperi(y_test=subset_test_y, testPred=subset_test_predictions,sig_level_2_prediction_intervals=subset_sig_level_2_prediction_intervals,sig_level_of_interest=sig_level_of_interest,plot_file_name=os.path.sep.join([calc_dir,'AD={}_Reg_Experi_vs_Pred_Plus_Err_Bars.tiff'.format(AD_subset)]),plot_title='')
                    
                else:
                    rmse,MAD,R2,Pearson,Pearson_Pval_one_tail,Spearman,Spearman_Pval_one_tail,validity, efficiency,ECE_new,ENCE,errorRate_s,no_compounds,scc = [None]*14
                    no_compounds = 0
                ...
                populateDictWithStats(dict_of_stats,AD_method_name,uncertainty_method,AD_subset,rmse,MAD,R2,Pearson,Pearson_Pval_one_tail,Spearman,Spearman_Pval_one_tail,validity, efficiency,ECE_new,ENCE,no_compounds,scc)
                
                
                
    statsFilesDict = writeStatsFiles(dict_of_stats,calc_dir,selected_ad_methods,selected_uncertainty_methods)
    
    print(f'RAN modellingEvalAnalysisForOneModelVersion(...) for update_version={update_version},random seed={global_random_seed}')
...
def main():
    ...
    for update_version in updates_in_order:
        ...
        for global_random_seed in all_global_random_seed_opts:
            ...
            ret_val = modellingEvalAnalysisForOneModelVersion(update_version,selected_uncertainty_methods,selected_ad_methods,ADMethod2Param,non_conformity_scaling,ml_alg,number_of_scp_splits,number_of_acp_splits,icp_calib_fraction,nrTrees,global_random_seed,id_col,smiles_col,ep_col,dataset_file,min_no_cmpds,sig_level_of_interest,all_sig_levels_considered_to_compute_ECE,native_uncertainty_alg_variant)
