...
from PublicDataModelling.general_purpose.common_globals import regression_dataset_names,ds_matched_to_ep_list
...
out_dir = os.path.sep.join([top_res_dir,'Summaries_All_EPs_Models'])
...
def filter_by_focus_endpoints(ds_matched_to_ep_list,list_of_endpoints_to_focus_on):

    new_ds_matched_to_ep_list = defaultdict(list)

    for ds in ds_matched_to_ep_list.keys():
        for ep in ds_matched_to_ep_list[ds]:
            if ep in list_of_endpoints_to_focus_on:
                print(f'Considering this endpoint = {ep}')
                new_ds_matched_to_ep_list[ds].append(ep)
            else:
                print(f'Skipping this endpoint={ep}')

    return new_ds_matched_to_ep_list
...
def compute_average_shift_metric_percentage_with_the_right_sign(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,metric):
    ...
    numerator = int(fraction_with_the_right_sign_str.split('/')[0])

    denominator = int(fraction_with_the_right_sign_str.split('/')[1])

    fraction_with_the_right_sign = float(numerator)/float(denominator)
    
    perc = round(100*fraction_with_the_right_sign,2)

    return perc,numerator,denominator
...
def compute_significant_percentage(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,p_vals_label_col,sig_label_of_interest):
    ...
    if not (rows_before_filtering==rows_after_filtering):
    ...
    numerator = sub_with_significant_p_vals_df.shape[0]

    denominator = rows_after_filtering

    perc = round(100*(numerator/denominator),2)
...
names_of_stats = ['%','total targets (or timesplits) meeting criteria','total targets (or timesplits) for which metric could be computed']

def update_dict_with_these_type_of_stats(type_of_stats,perc,numerator,denominator,dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,ds,test_set_type,metric):

    for stat_name in names_of_stats:

        if '%' == stat_name:
            stat_val = perc
        elif 'total targets (or timesplits) meeting criteria' == stat_name:
            stat_val = numerator
        elif 'total targets (or timesplits) for which metric could be computed' == stat_name:
            stat_val = denominator
        else:
            raise Exception(f'Unrecognised stat_name={stat_name}')
        
        dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig[ds][test_set_type][metric][f'{type_of_stats} : {stat_name}'] = stat_val
...
def analyse_shift_metric_signs_and_p_values(dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,shift_metric_endpoints_to_all_fold_seed_in_and_out_ad_vals,ds,metric,test_set_type,p_values_info,p_vals_label_col=p_vals_label_col,endpoint_col=endpoint_col,ad_subset_col=ad_subset_col,raw_one_tail_sig_label="*",adj_one_tail_sig_label="**",raw_two_tail_sig_label="x",adj_two_tail_sig_label="xx"):
    ...
    type_of_stats = 'no. metric values'
    expected_result_types.append(type_of_stats)

    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig[ds][test_set_type][metric][type_of_stats] = compute_number_of_non_missing_metrics(sub_df=average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,metric=val_col)

    #----------------------
    
    type_of_stats = 'Average shift-metric has the right sign'
    for stat_name in names_of_stats:
        expected_result_types.append(f'{type_of_stats} : {stat_name}')

    perc,numerator,denominator = compute_average_shift_metric_percentage_with_the_right_sign(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,metric)
    
    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig = update_dict_with_these_type_of_stats(type_of_stats,perc,numerator,denominator,dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,ds,test_set_type,metric)

    #---------------------

    type_of_stats = 'Raw one-tail p-value is significant [average shift-metric has right sign]'
    for stat_name in names_of_stats:
        expected_result_types.append(f'{type_of_stats} : {stat_name}')
    
    perc,numerator,denominator = compute_significant_percentage(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,p_vals_label_col,sig_label_of_interest=raw_one_tail_sig_label)

    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig = update_dict_with_these_type_of_stats(type_of_stats,perc,numerator,denominator,dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,ds,test_set_type,metric)
    
    #------------------------------------
    
    type_of_stats = 'Adjusted one-tail p-value is significant [average shift-metric has right sign]'
    for stat_name in names_of_stats:
        expected_result_types.append(f'{type_of_stats} : {stat_name}')
    
    perc,numerator,denominator = compute_significant_percentage(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,p_vals_label_col,sig_label_of_interest=adj_one_tail_sig_label)

    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig = update_dict_with_these_type_of_stats(type_of_stats,perc,numerator,denominator,dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,ds,test_set_type,metric)

    #----------------------------------
    
    type_of_stats = 'Raw two-tail p-value is significant [average shift-metric has wrong sign]'
    for stat_name in names_of_stats:
        expected_result_types.append(f'{type_of_stats} : {stat_name}')
    
    perc,numerator,denominator = compute_significant_percentage(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,p_vals_label_col,sig_label_of_interest=raw_two_tail_sig_label)
    
    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig = update_dict_with_these_type_of_stats(type_of_stats,perc,numerator,denominator,dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,ds,test_set_type,metric)

    #----------------------------------
    
    type_of_stats = 'Adjusted two-tail p-value is significant [average shift-metric has wrong sign]'
    for stat_name in names_of_stats:
        expected_result_types.append(f'{type_of_stats} : {stat_name}') 
    
    perc,numerator,denominator = compute_significant_percentage(average_shift_metrics_matched_to_p_val_labels_for_average_shift_metrics_df,val_col,p_vals_label_col,sig_label_of_interest=adj_two_tail_sig_label)
    ...
    return dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,expected_result_types


def convert_results_to_df(dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,expected_result_types):
    ...
    for ds in dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig.keys():
        for test_set_type in dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig[ds].keys():
            for metric in dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig[ds][test_set_type].keys():
                ...
                for res_type in expected_result_types:
...
def main():
    ...
    if not list_of_endpoints_to_focus_on is None:
        ds_matched_to_relevant_ep_list = filter_by_focus_endpoints(ds_matched_to_ep_list,list_of_endpoints_to_focus_on)
    else:
        ds_matched_to_relevant_ep_list = ds_matched_to_ep_list

    dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig = neverEndingDefaultDict()

    for dataset in ds_matched_to_relevant_ep_list.keys():
        ...
        metrics_matched_to_test_set_type_to_endpoints_to_all_fold_seed_in_and_out_ad_vals = get_metrics_matched_to_test_set_type_to_endpoints_to_all_fold_seed_in_and_out_ad_vals(dataset,ds_matched_to_relevant_ep_list,merged_raw_stats_file,regression_dataset_names,classification_stats,regression_stats,ep_type_matched_to_default_AD_uncertainty_methods,endpoint_col,test_set_type_col,fold_col,rnd_seed_col,alg_col,ad_col,ad_subset_col)
        ...
        for metric in metrics_matched_to_test_set_type_to_endpoints_to_all_fold_seed_in_and_out_ad_vals.keys():
            ...
            for test_set_type in metrics_matched_to_test_set_type_to_endpoints_to_all_fold_seed_in_and_out_ad_vals[metric].keys():
                ...
                dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,expected_result_types = analyse_shift_metric_signs_and_p_values(dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,shift_metric_endpoints_to_all_fold_seed_in_and_out_ad_vals,dataset,metric,test_set_type,p_values_info)
                #########################################
    
    
    res_df = convert_results_to_df(dict_of_ds_to_test_set_type_to_metric_to_proportions_with_right_sign_and_one_tail_stat_sig,expected_result_types)
